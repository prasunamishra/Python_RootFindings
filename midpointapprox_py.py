# -*- coding: utf-8 -*-
"""MidPointApprox.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ue5PK1b4gzLWGKjjS7VToPSJojwQ2dlS
"""

#MidPointApprox
import numpy as np
import matplotlib.pyplot as plt

def plot_function(func, a, b):
    """
    This function plots the graph of the input func
    within the given interval [a,b).
    """
    x = np.linspace(a, b, 1000)
    y = func(x)
    plt.plot(x, y)
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.title('Graph of the Function')
    plt.grid(True)
    plt.show()

def midpoint_approx(func, a, b, N):
    '''Compute the Midpoint Approximation of Definite Integral of a function over the interval [a,b].

    Parameters
    ----------
    func : function
           Vectorized function of one variable
    a , b : numbers
        Endpoints of the interval [a,b]
    N : integer
        Number of subintervals of equal length in the partition of [a,b]

    Returns
    -------
    float
        Approximation of the definite integral by Midpoint Approximation.
    '''
    h = (b - a) / N
    result = 0
    for i in range(N):
        result += func(a + (i + 0.5) * h)
    result *= h
    return result

if __name__ == "__main__":
    # 1st Function to be integrated
    func_1 = lambda x: x / (x**2 + 1)

    # Indefinite Integral of the function
    antiderivative_1 = lambda x: 0.5 * np.log(x**2 + 1)

    # End points for 1st Function
    a1 = 0
    b1 = 5

    # Call the function to Plot the graph of the functions
    plot_function(func_1, a1, b1)

    # Number of partitions for 1st Function
    N1 =500

    # Call midpoint_approx to compute Midpoint Approximation for 1st Function
    midpoint_approx_1 = midpoint_approx(func_1, a1, b1, N1)

    # Calculate the true value of the definite integral for 1st Function
    definite_integral_1 = antiderivative_1(b1) - antiderivative_1(a1)

    # Calculate the absolute error for 1st Function
    error_1 = np.abs(midpoint_approx_1 - definite_integral_1)

    print("Subinterval width = {:0.6f}".format((b1 - a1) / N1))
    print("Midpoint Approximation for 1st Function = {:0.6f}".format(midpoint_approx_1))
    print("Actual Value for 1st Function = {:0.6f}".format(definite_integral_1))
    print("Absolute error for 1st Function = {:0.8f}".format(error_1))

import numpy as np
import matplotlib.pyplot as plt

def plot_function(func, a, b):
    """
    This function plots the graph of the input func
    within the given interval [a,b).
    """
    x = np.linspace(a, b, 1000)
    y = func(x)
    plt.figsize=(10,4)
    plt.plot(x, y)
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.title('Graph of the Function')
    plt.grid(True)
    plt.show()

def midpoint_approx(func, a, b, N):
    '''Compute the Midpoint Approximation of Definite Integral of a function over the interval [a,b].

    Parameters
    ----------
    func : function
           Vectorized function of one variable
    a , b : numbers
        Endpoints of the interval [a,b]
    N : integer
        Number of subintervals of equal length in the partition of [a,b]

    Returns
    -------
    float
        Approximation of the definite integral by Midpoint Approximation.
    '''
    h = (b - a) / N
    result = 0
    for i in range(N):
        result += func(a + (i + 0.5) * h)
    result *= h
    return result

if __name__ == "__main__":
    # 2nd Function to be integrated
    func_2 = lambda x: np.exp(x)

    # Indefinite Integral of the function
    antiderivative_2 = lambda x: np.exp(x)

    # End points for 2nd Function
    a2 = 0
    b2 = 5

    # Call the function to Plot the graph of the functions
    plot_function(func_2, a2, b2)

    # Number of partitions for 2nd Function
    N2 = 500

    # Call midpoint_approx to compute Midpoint Approximation for 2nd Function
    midpoint_approx_2 = midpoint_approx(func_2, a2, b2, N2)

    # Calculate the true value of the definite integral for 2nd Function
    definite_integral_2 = antiderivative_2(b2) - antiderivative_2(a2)

    # Calculate the absolute error for 2nd Function
    error_2 = np.abs(midpoint_approx_2 - definite_integral_2)

    print("Subinterval width = {:0.6f}".format((b2 - a2) / N2))
    print("Midpoint Approximation for 2nd Function = {:0.6f}".format(midpoint_approx_2))
    print("Actual Value for 2nd Function = {:0.6f}".format(definite_integral_2))
    print("Absolute error for 2nd Function = {:0.8f}".format(error_2))